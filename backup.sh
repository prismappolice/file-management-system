...
PG_USER="your_username"  # Change as needed
PG_DB="your_database"    # Change as needed
PG_PASSWORD="your_password" # Change as needed or use .pgpass for security

# Export password for non-interactive use (optional, not recommended for production)
export PGPASSWORD="$PG_PASSWORD"

pg_dump -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" -F c -b -v -f "$BACKUP_DIR/pgdb_$DATE.backup" "$PG_DB"
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ Database backed up: pgdb_$DATE.backup${NC}"
else
...
DB_SIZE=$(du -h "$BACKUP_DIR/pgdb_$DATE.backup" 2>/dev/null | cut -f1)
...
find $BACKUP_DIR -name "pgdb_*.backup" -mtime +$RETENTION_DAYS -delete
