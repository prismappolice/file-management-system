...
router.post('/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    const { fileNo, subject, department, date, program, memo_id } = req.body;
    ...
    console.log('Memo ID:', memo_id);

    // Insert into database - PostgreSQL uses lowercase column names by default
    const sql = `INSERT INTO files (fileno, subject, department, date, filename, filepath, program, created_by, memo_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id`;
    
    try {
      const result = await pool.query(sql, [fileNo, subject, department, date, filename, filepath, programName, createdBy, memo_id || null]);
...
router.get('/files', async (req, res) => {
  ...
  console.log('Files request - Program:', program, 'Memo ID:', memo_id, 'UserType:', userType, 'UserName:', userName);
  
  let sql = `SELECT id, fileno, subject, department, date, filename, program, created_by, uploaded_at, memo_id FROM files`;
  let params = [];
  let whereClauses = [];
  let paramIndex = 1;
  ...
  // Filter by memo_id if specified
  if (memo_id) {
    whereClauses.push(`memo_id = $${paramIndex++}`);
    params.push(memo_id);
  }
